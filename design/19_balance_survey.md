...
## 8. QA 중심 개선안: 역할 기반 우선순위 & 테스트 시나리오

본 장의 수치 분석을 바탕으로, bmad-gds 작업 흐름 기준 역할 우선순위를 다음과 같이 제안한다.

### 8.1 역할 기준 우선순위

| 우선순위 | bmad-gds 역할 | 산출물 | 목적 | 이유 |
|---|---|---|---|---|
| P0 | **QA/테스트** | 플레이테스트 스크립트, 수용 기준 | 진행 차단 이슈 즉시 차단 | SeedPod 미소비, HoldOut/3성 모순, 실패 보존율 계산 오류 등은 플레이 경험을 직접 무너뜨림 |
| P1 | **게임 밸런스(디자인)** | `design/19`, `design/06` 수치 정책 | 초기 경제 긴장감, 후반 진입 허들, 목표 난이도 조율 | BD/Run 과잉, 스타 요구량 선형성, 3성 기준 불일치 등 설계 축 조정 |
| P1 | **코드/시스템(Implementation)** | `DistrictBalanceDefaults`, `RewardCalculator`, `ExpeditionDirector` | 설계값 반영 및 회귀 억제 | 분석값과 실제 실행값 불일치 최소화 |
| P2 | **아트/UV(검증)** | 캐릭터 UV 사양, 텍스처/프리펩 매핑 | 아바타와 픽업 비주얼의 시각 안정성 | 캐릭터 UV 레이어 누락 시 렌더 이슈/보정 비용 증가 |
| P2 | **운영/릴리스** | QA 체크리스트, 릴리스 노트 | 출시 전 누락 점검 완료 | 자원 루프/튜토리얼/저장 회귀가 운영 단계에서 드러나기 쉬움 |

### 8.2 QA 테스트 시나리오 (권고)

| ID | 시나리오 | 수행 방식 | 기대 결과 |
|---|---|---|---|
| QA-01 | **경제 수렴 테스트** | 새 저장 상태에서 Dock 1~3회 연속 클리어 | Dock 진입 후 2회 이내엔 과도한 잉여 BD가 누적되지 않아야 하며, 다음 지구로 넘어갈 동기가 남아야 함 |
| QA-02 | **스타 요구량 체감 테스트** | Glass Narrows/Sunken/Lighthouse 도달 조건 반복 달성 | 1성만으로 후반 지구 접근이 어려워야 하며, 2~4회 달성 루프가 설계 의도대로 작동 |
| QA-03 | **Lighthouse HoldOut/3성 검증** | Lighthouse Crown 3성 타임컷 내 Playtest 20회 | 3성 달성은 논리적으로 가능해야 하며(기록/로그로 재현), 특정 수치 조합에서 영구적으로 불가하지 않아야 함 |
| QA-04 | **실패 보존율 회귀** | 실패 런에서 자원 70% 기준 적용 확인 | BloomDust/ Scrap은 설계/구현 일치, CleanWater·MemoryPearl은 정책대로 처리 |
| QA-05 | **SeedPod 루프 검증** | SeedPod 누적량을 3회 이상 수집 가능한 구간 반복 플레이 | 수집한 SeedPod의 사용처가 UI/허브/업그레이드에서 감소 흐름으로 반영되어야 함(소비점 미구현 시 개선 항목으로 재기입) |
| QA-06 | **맵 경계/지도 이탈 회귀** | 맵 바깥 방향으로 지속 이동 후 UI/결과 복귀 | 경계 미구현 구간에서는 회수 불가 이탈이 허용되지 않거나, 회복 수단이 준비돼야 함 |
| QA-07 | **캐릭터 UV 맵 누락 점검** | 플레이어 프리팹(`Art/Characters/player_avatar`)의 Mesh UV 채널 0 존재 여부 확인 | UV가 비어 있으면 실패 처리: 임시 UV 프록시 또는 텍스처 재임포트 후 재검증 |

### 8.3 캐릭터 UV 맵 누락 점검 항목 (디자인-아트 합동)

1. `Art/Characters/player_avatar`가 실제 메시/메시 배열에서 UV 채널을 보유하는지 확인한다.
2. UV 채널이 비어 있거나 0/1만 존재할 경우, 경량 UV 템플릿(`T_Player_Diffuse`, `T_Player_Emission`)과 정합성을 맞춰 즉시 보정한다.
3. `RuntimeArtDirector`가 텍스처 탐색 후 `_MainTex` 바인딩 실패 시, 아티팩트 없이 기본 색상 표시가 가능한지 확인한다.
4. UV 임포트 세팅에서 `Generate Lightmap UVs`, `Optimize Mesh` 옵션이 켜져 있더라도 채널 충돌이 없는지 확인한다.
5. 최종 확인 기준: 플레이 화면에서 캐릭터 바닥면/상체 텍스처 번짐, 알파 홀, 극단적 밴딩이 없어야 함.

### 8.4 즉시 실행 패키지 (1차 스프린트)

- **핵심 고정(P0-P1):** SeedPod 소비처 정의 + 3성 수학 모순 재검증 + 실패 보존율 회귀 테스트 정비.
- **후속(P2):** 캐릭터 UV 누락 체크를 QA 패키지에 상시 항목으로 추가하고, `design/21`의 아트 승인 루프에 반영.

*최종 수정: 2026-03-22 | 참조: DistrictBalanceDefaults.cs, RewardCalculator.cs, design/06, design/07, design/17*


## 8. QA-주도 밸런스 개선안 (bmad-gds 역할 우선순위)

> 목적: 현재 밸런스 분석 결과를 테스트 관점으로 재구조화하여, `docs/qa_verification_checklist.md`에서 바로 실행 가능한 시나리오로 연결.

### 역할별 우선순위 (RACI)

| 우선순위 | 소유 역할 (bmad-gds) | 목표 | 1차 실행 항목 | 완료 조건 |
|---------|---------------------|------|---------------|-----------|
| P0 | **QA Lead** + **Designer** | 출시 전 즉시 막기 | `SeedPod` 소비처 누락 위험, 런 후 자원 보존/진행 장벽 불일치, 목표 난이도/3성 상호 모순 재검증 | 1) 테스트 실패율 0% 또는 치명 이슈 해결 문서화, 2) 체크리스트 항목 추가 후 재실행 |
| P1 | **Architect** + **Designer** | 장기 진행 동기 및 재도전 루프 회복 | 스타 요구량 비선형 검증, Dock→Reed Fields 난이도 점프(초/픽업) 완화, Harbor Pump/Route Scanner/Resonator 비용/효과 도달성 분석 | 1) 런당 목표 지표(달성률/2~3성 비율) 범위 유지, 2) 후반 지구 진입 조건이 반복 플레이를 유도 |
| P2 | **Developer** + **QA** | 구현 안정성 | `docs/qa_verification_checklist` 기반 아트·씬 경계·보존율·입력 방지 시나리오 회귀 | 2개 이상의 플랫폼에서 동일 결과 재현 및 회귀 실패 없음 |

### QA 관점 개선 권고

1. **밸런스 검증 중심 이동**: 스펙 변경은 바로 수치만 수정하는 방식이 아니라, 최소 10회 QA 세션에서 산출되는 지표(달성률, 자원 보존율, 3성 달성률)로 승인 후 반영.
2. **재현성 있는 결함 보고**: 이슈는 항상 `재현 단계 / 기대값 / 실제값 / 영향도 / 수정 우선순위`를 고정 포맷으로 기록.
3. **아트-밸런스 연계 검증 추가**: 캐릭터 UV/재질 품질 이슈가 자원/난이도 수치에는 영향을 안 주더라도, 플레이 체감 난이도와 시야 판단 오류를 만든다면 QA P0로 승격.
4. **체크리스트 동기화 원칙**: `design/19`의 개선안은 `docs/qa_verification_checklist.md`에 1:1로 연결되는 테스트 항목(체크박스) 형태로 유지.

### 즉시 실행 테스트 큐 (이번 릴리스)

- [ ] **[P0] SeedPod dead stock 점검**: Reed Fields 이상에서 수집량 누적 후 사용처 미제공으로 지표가 폭증하는지 확인.
- [ ] **[P0] 난이도 수학 모순 점검**: HoldOut 목표(Glass Narrows/Lighthouse Crown) 달성 시 3성 시간 컷이 실제 통과 가능한지 플레이 로그로 검증.
- [ ] **[P1] Dock→Reed Fields 난이도 점프 검증**: 튜토리얼 종료 10분 내 첫 이탈률 계산, 80% 이하를 목표로 튜닝 시나리오 적용.
- [ ] **[P1] 스타 요구량 후반 반영성 점검**: Lighthouse Crown 진입 전후 평균 런 수, 2~3성→3성 전환비 비교.
- [ ] **[P2] 실패 리텐션 점검**: `ResourceRetention` 계산이 난이도별로 설계 의도(70% 수집 보존)와 일치하는지 재확인.
- [ ] **[P2] 캐릭터 UV맵 결함 탐지**: 플레이어 모델에서 텍스처 샘플링 아티팩트(뒤집힘·늘어짐·이음선 누락) 발생률을 확인.

---

## 9. Character UV Map 누락 리스크 항목

### 9.1 캐릭터 UV 이슈의 QA 의미

- **발생 가능성**: 현재 `player_avatar`는 외부 FBX 에셋 기반(`Leopard.fbx`)이며, 텍스처 재바인딩 경로가 변경될 경우 `UV` 채널 mismatch가 UI/디버그 없이도 표면 깜박임, 스킨 얼룩, 윤곽 누락으로 표출될 수 있다.
- **발생 조건**: `RuntimeArtDirector.NormalizeRendererMaterials()`가 Material 정규화 시 샘플러/텍스처 재배치가 일어날 때, UV가 없는 상태나 알파 채널 mismatch가 있으면 시야 판정이 흔들려 난이도 체감도 저하.
- **릴리즈 영향도**: 난이도/경제 밸런스와 무관해 보여도, 시각 판정 오류(픽업 거리 판단, 이동 방향성)을 유발해 QA 실패 확률이 높다.

### 9.2 UV 점검 테스트(디자인 문서 반영)

| 항목 | 기준 | 증빙 방식 |
|------|------|----------|
| UV 채널 존재성 | 채널 0/1 정상 임포트 | FBX Importer 설정 스크린샷 + 씬 뷰 스냅샷 |
| UV 품질 | 1회 런에서 UV 누수/뒤집힘 0건 | 12컷 스크린샷 + QA 판정표 |
| UV 재임포트 안정성 | 런타임에서 텍스처 바인딩 깨짐 0건 | 런타임 체크리스트 재실행 |
| 문서 추적성 | 본 항목이 design/19 항목에서 QA 체크리스트로 연결 | `docs/qa_verification_checklist.md` 항목 10 참조 |

### 9.3 우선순위 반영

- **P0**: UV 아티팩트가 플레이 지표(픽업 접근, 시선 이동, 거리 감지)에 직접 영향을 줄 경우 즉시 수정.
- **P1**: 시각적 불일치가 미미하더라도 자산 패키지 교체 시 재점검 항목으로 유지.
- **P2**: 정상 동작 확인 후 문서 주기성 감사만 수행.
